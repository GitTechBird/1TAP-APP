...
# Custom Made
def create_item_template(activity):
    ...
    # Create item group
    route = "services/company-setup/chemical-trading"
    frappe.get_doc({
		"doctype": "Item Group",
		"item_group_name": "New Trading",
		"parent_item_group": "Company setup",
        "old_parent": "Company setup",
		"route": route,
    })

    # Create item template
    template_name = f"{country_code}_{zone_name}_CompanySetup_{activity_group}"
    frappe.get_doc(
        {
            "doctype": "Item",
            ...
            "has_variants": 1,
            "attributes": [{"attribute": attr.attribute} for attr in activity_attributes]
        }
    ).insert()
...
def create_and_update_item_price(template_name, activity):
	items = frappe.get_all(doctype = "Item", filters={"variant_of": template_name}, fields=["name","item_name"])
	zone_name = "International Free Zone Authority"
	for i in items:
...
def create_variants_helper():
    latest_entry = frappe.get_list("OT Business Activity", limit=1, order_by="creation desc")
    if latest_entry:
        activity = frappe.get_doc("OT Business Activity", {"name": latest_entry[0].name})
        # activity related info
        activity_attributes = activity.activity_item_attributes
        template_name = "United Arab Emirates_International Free Zone Authority_CompanySetup_Services"
        args = {}
        ...
        return create_and_update_item_price(template_name, activity)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
@frappe.whitelist()
def create_variants_custom():
    return create_variants_helper()

@frappe.whitelist()
def create_item_price():
	template_name ="United Arab Emirates_International Free Zone Authority_CompanySetup_Services"
	activity = frappe.get_doc("OT Business Activity", {"name": "48bb9c7589"})
	return create_and_update_item_price(template_name, activity)

@frappe.whitelist()
def create_item_group():
    # Create item group
    route = "services/company-setup/new-trading"
    frappe.get_doc({
		"doctype": "Item Group",
		"item_group_name": "New Trading",
		"parent_item_group": "Company setup",
...
def get_info():
    ...
    if latest_entry:
        ...
        # activity related info
        activity_attributes = activity.activity_item_attributes
        res=activity_attributes
    return res

@frappe.whitelist()
def get_items_latest():
	items = frappe.get_list(doctype="Item", fields=["item_name"])
	for item in items:
		item = frappe.get_doc(doctype="Item", filters={"item_name": item.item_name})
		return item
	return items