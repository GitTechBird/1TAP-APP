...
def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	...
	if not otp:
		...
		if two_factor_is_enabled_for_(login_manager):
			return False
			# return "two_factor_is_enabled_for_"
		return True
		# return f"True two_factor_is_enabled_for_"
	if not tmp_id:
		tmp_id = frappe.form_dict.get("tmp_id")
	...
	tracker = get_login_attempt_tracker(login_manager)
	...
	if hotp_token:
		if hotp.verify(otp, int(hotp_token)):
			...
			return True
			# return f"True hotp, {hotp}"
		else:
			tracker.add_failure_attempt()
			return False
			# return f"hotp_token, {hotp}"
	...
	if totp.verify(otp):
		...
		if not get_default(login_manager + "_otplogin"):
			set_default(login_manager + "_otplogin", 1)
			delete_qrimage(login_manager)
		tracker.add_success_attempt()
		return True
		# return f"True totp, {totp}"
	else:
		tracker.add_failure_attempt()
		return False
		# return f"totp, {totp}"
...
# Custom
def validate_otp_helper(user_email, otp, tmp_id):
    try:
