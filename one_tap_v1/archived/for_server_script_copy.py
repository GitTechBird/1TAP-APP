...
def create_template(self):
    # latest_entry = frappe.get_list("OT Business Activity", limit=1, order_by="creation desc")
    latest_entry = self.get_doc_before_save()
    if latest_entry:
        activity = frappe.get_doc("OT Business Activity", latest_entry[0].name)
        # activity related info
        activity_attributes = activity.activity_item_attributes
        activity_group = activity.business_activity_group_name
        # zone info
        zone_name = activity.business_zone
        zone = frappe.get_doc("OT Zone Master", {"zone_name": zone_name})
        # state info
        ...
        # Check if item group exists, else create it
        item_group_exists = frappe.db.exists('Item Group', activity_group)
        if not item_group_exists:
            frappe.get_doc({
                "doctype": "Item Group",
                "item_group_name": activity_group,
                "parent_item_group": "Company setup", # or another appropriate parent group
                "is_group": 0  # Set to 1 if this is a parent group itself
            }).insert()

        # Create item template
        template_name = f"{country_code}_{zone.zone_code}_CompanySetup_{activity_group}"
        attribute_list = []
        for attr in activity_attributes:
            attr_doc = frappe.get_doc("Item Attribute", attr.attribute)
            if attr_doc.numeric_values:
                # Ensure that from_range, to_range, and increment have default values if None
                from_range = attr_doc.from_range or 0
                to_range = attr_doc.to_range or 0
                increment = attr_doc.increment or 1  # Assuming the default increment is 1

                attribute_list.append({
                    "attribute": attr_doc.name,
                ...
                # If the attribute is not numeric, append only the attribute name
                attribute_list.append({
                    "attribute": attr_doc.name
                })

		frappe.log_error(f"An error occurred:")
        ...
        #             "attributes": attribute_list
        #         }).insert()
        #     # If the insert is successful, you can use the new_item as needed
        #     frappe.msgprint(msg='Item created successfully', title='Success', indicator='green')
        # except frappe.DuplicateEntryError as e:
        #         frappe.msgprint(msg=f'An item with this code or name already exists: {e}', title='Duplicate Entry', indicator='orange')    
        # except Exception as e:
        #     frappe.msgprint(msg=f'An unexpected error occurred: {e}', title='Error', indicator='red')
        # #````````````````````````````````````````````````````````#
		
        # # Update activity with item generated code
        # activity.item_generated_code = template_name
        # activity.save()
        # # Creating Variants and further processes
        # args = {}
        # for attr in activity_attributes:
        #     if attr.attribute == "OT Visa Years attributes":
        #         update_args(attr, args)
        #     if attr.attribute == "OT Visa Number attributes":
        #         update_args(attr, args)
        # create_multiple_variants(template_name, args)
        # # Create Item Price and update value for item_price
        # create_and_update_item_price(template_name, activity, zone_name)
...
def create_and_update_item_price(template_name, activity, zone_name):
	items = frappe.get_all(doctype = "Item", filters={"variant_of": template_name}, fields=["name","item_name"])
	for i in items:
		item = frappe.get_doc("Item", i.name)
		## Getting the price of the item
		activity_price = activity.activity_additional_price if activity.activity_additional_price else 0
