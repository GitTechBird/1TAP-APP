...
def create_and_update_item_price(template_name, activity):
	items = frappe.get_all(doctype = "Item Price", filters={"variant_of": template_name}, fields=["item_name"])
	zone_name = "IFZA"
	for item in items:
		## Getting the price of the item
		activity_price = activity.activity_additional_price			
		# Getting the price from the OT Zone Master
		item_attributes = frappe.get_value(doctype="Item", filters={"item_name":item.item_name}, fields=["attributes"])
		visa=0
		year=1
...
def create_variants_custom():
    ...
    if latest_entry:
		...
		create_and_update_item_price(template_name, activity)