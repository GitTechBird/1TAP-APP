...
ACTIVITY_TYPES = ["General Trading", "Regular Activity", "Special Activity"]
VISA_MIN = 0
VISA_MAX = 4
YEAR_MIN = 1
YEAR_MAX = 5
...
def example():
    ...
    for visa in range(VISA_MIN, VISA_MAX+1):
        for year in range(YEAR_MIN, YEAR_MAX+1):
            ...
            if zone.get(key) and zone[key]:
                visa_years.add(str(year))
                visa_numbers.add(str(visa))
    args = {
        "OT Visa Years attributes": list(visa_years),
        "OT Visa Number attributes": list(visa_numbers),
    }
    print(args)
    count = 0
    ...
    attribute_list = []
    visa_year_attr = frappe.get_doc("Item Attribute", "OT Visa Years attributes")
    visa_num_attr = frappe.get_doc("Item Attribute", "OT Visa Number attributes")
...
def create_template():
    ...
    for name in zones:
        ...
        for activity_type in ACTIVITY_TYPES:
            template_name = f"{zone['zone_code']}_{activity_type}"
            # Update Attributes
            attribute_list = []
            visa_year_attr = frappe.get_doc("Item Attribute", "OT Visa Years attributes")
            visa_num_attr = frappe.get_doc("Item Attribute", "OT Visa Number attributes")
            ...
            if not item_template_exists:
                ...
                create_and_update_item_price(template_name, zone['zone_name'])
                frappe.db.commit()
                time.sleep(30)
...
def create_template_1():
    ...
    for name in zones:
        ...
        zone = zone.as_dict()
        for activity_type in ACTIVITY_TYPES:
            template_name = f"{zone['zone_code']}-{activity_type}"
            # Update Attributes
            attribute_list = []
            visa_year_attr = frappe.get_doc("Item Attribute", "OT Visa Years attributes")
            visa_num_attr = frappe.get_doc("Item Attribute", "OT Visa Number attributes")
            ...
            if not item_template_exists:
                ...
                for visa in range(VISA_MIN, VISA_MAX+1):
                    for year in range(YEAR_MIN, YEAR_MAX+1):
                        key = f"{visa}visa_{year}year"
                        if zone.get(key) and zone[key]:
                            attribute_values = {'OT Visa Years attributes': str(year), 'OT Visa Number attributes': str(visa)}
...
# Create template test # Item group mandatory
def create_template_test():
    for activity_type in ACTIVITY_TYPES:
        template_name = f"TestCode-{activity_type}"
        # Update Attributes
        attribute_list = []
        visa_year_attr = frappe.get_doc("Item Attribute", "OT Visa Years attributes")
        visa_num_attr = frappe.get_doc("Item Attribute", "OT Visa Number attributes")
        ...
        if not item_template_exists:
            ...
            create_multiple_variants(template_name, args)
            # Create Item Price and update value for item_price
            create_and_update_item_price(template_name, "Test Name")
            frappe.db.commit()
            # time.sleep(30)
            return "Item Generated Successfully"

def create_template_test_1():
    for activity_type in ACTIVITY_TYPES:
        template_name = f"Testing Code-{activity_type}"
        # Update Attributes
        attribute_list = []
        visa_year_attr = frappe.get_doc("Item Attribute", "OT Visa Years attributes")
        visa_num_attr = frappe.get_doc("Item Attribute", "OT Visa Number attributes")
                ...
                for year in range(YEAR_MIN, YEAR_MAX+1):
                    ...
                    if zone.get(key) and zone[key]:
                        ...
                        if not get_variant(template_name, args=attribute_values):
                            ...
                            variant.save()
            # Create Item Price and update value for item_price
            create_and_update_item_price(template_name, "Testing ")
            frappe.db.commit()
            # time.sleep(30)
            return "Item Generated Successfully"
...
def create_and_update_item_price(template_name, zone_name):
