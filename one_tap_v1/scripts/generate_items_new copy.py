...
# Custom Made
def create_template(activity_doc_name):
    # latest_entry = frappe.get_list("OT Business Activity", limit=1, order_by="creation desc")
    # latest_entry = self.get_doc_before_save()
    # if latest_entry:
        # activity = frappe.get_doc("OT Business Activity", activity_group_name)
        activity = frappe.get_doc("OT Business Activity", activity_doc_name)
